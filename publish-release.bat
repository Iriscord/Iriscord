@echo off
:: ─── Luacord — Publier une nouvelle release sur GitHub ─────────────────────
...
if "%NOTES%"=="" set NOTES=Luacord %VERSION%

:: Chemins de sortie
set DIST_DIR=dist\desktop
set OUT_DIR=release\installer
set DIST_ZIP=%OUT_DIR%\luacord-dist.zip
set INSTALLER_EXE=%OUT_DIR%\Luacord-Installer.exe
set VERSION_JSON=%OUT_DIR%\version.json
set DESKTOP_ASAR=dist\desktop.asar

echo.
echo  ╔═══════════════════════════════════════════════════╗
echo  ║    LUACORD — Publication release v%VERSION%
echo  ╚═══════════════════════════════════════════════════╝
echo.
...
:: ── 5. Compiler Luacord-Installer.exe ──────────────────────────────────────
echo.
echo  [5/8] Compilation de Luacord-Installer.exe...
...
if not exist "%INSTALLER_EXE%" (
    echo  [ERREUR] Luacord-Installer.exe introuvable apres compilation.
    pause
    exit /b 1
)

for %%F in ("%INSTALLER_EXE%") do echo  [5/8] Luacord-Installer.exe cree (%%~zF octets)

:: ── 6. Créer luacord-dist.zip ──────────────────────────────────────────────
echo.
echo  [6/8] Creation de luacord-dist.zip...
...
powershell -NoProfile -Command "Add-Type -Assembly System.IO.Compression.FileSystem; $src = (Resolve-Path '%DIST_DIR%').Path; $dst = (Join-Path (Resolve-Path 'release\installer').Path 'luacord-dist.zip'); [System.IO.Compression.ZipFile]::CreateFromDirectory($src, $dst, [System.IO.Compression.CompressionLevel]::Optimal, $false)"

if not exist "%DIST_ZIP%" (
    echo  [ERREUR] Impossible de creer luacord-dist.zip
    pause
    exit /b 1
)

for %%F in ("%DIST_ZIP%") do echo  [6/8] luacord-dist.zip cree (%%~zF octets)
...
(
    ...
    echo   "installerUrl": "https://github.com/luacordoff/luacord/releases/latest/download/Luacord-Installer.exe",
    echo   "distUrl": "https://github.com/luacordoff/luacord/releases/latest/download/luacord-dist.zip",
    echo   "downloadUrl": "https://github.com/luacordoff/luacord/releases/latest/download/desktop.asar",
...
gh release create "v%VERSION%" ^
    "%INSTALLER_EXE%#Luacord-Installer.exe" ^
    "%DIST_ZIP%#luacord-dist.zip" ^
    "%DESKTOP_ASAR%#desktop.asar" ^
    "%VERSION_JSON%#version.json" ^
    --repo luacordoff/luacord ^
    --title "Luacord v%VERSION%" ^
    --notes "!NOTES!" ^
    --latest
...
echo.
echo  ╔═══════════════════════════════════════════════════════════════╗
echo  ║  Luacord v%VERSION% publie avec succes !
echo  ║
echo  ║  Fichiers publies sur GitHub :
echo  ║    Luacord-Installer.exe    — installeur .exe avec GUI
echo  ║    luacord-dist.zip         — JS obfusques (pour l'injec.)
echo  ║    version.json               — metadonnees de version
echo  ║
echo  ║  Les utilisateurs telechargeront Luacord-Installer.exe
