# build-installer.ps1 - Compile Luacord-Installer.exe
...
$OutExe = Join-Path $OutDir "Luacord-Installer.exe"

Write-Host ""
Write-Host "  [Luacord] Compiling installer..." -ForegroundColor Cyan
...
if ($hasDotnet) {
    ...
    & dotnet publish "$SrcDir\LuacordInstaller.csproj" `
        -c Release `
        -o $OutDir `
        --nologo `
        -v quiet `
    ...
    $built = Get-ChildItem $OutDir -Recurse -Filter "Luacord-Installer.exe" -ErrorAction SilentlyContinue | Select-Object -First 1
...
# Method 2: csc.exe (.NET Framework - always present on Windows)
    ...
    $ico    = Join-Path $Root "luacord.ico"
...
if (Test-Path $OutExe) {
    ...
    Write-Host ""
    Write-Host "  OK  Luacord-Installer.exe compiled ($size KB)" -ForegroundColor Green
    Write-Host "    -> $OutExe" -ForegroundColor DarkGray
    Write-Host ""
} else {
    Write-Host "  [ERROR] Luacord-Installer.exe not found after compilation." -ForegroundColor Red
    exit 1
}
