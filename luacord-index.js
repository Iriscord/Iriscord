...
// ── CRITIQUE : userData = dossier Luacord pour les settings/plugins
...
const luacordData = path.join(app.getPath("appData"), "Luacord");
app.setPath("userData", luacordData);
...
// pour le token Discord stable. Quand Luacord charge ce cache, il lit des permissions
...
// AppUserModelId unique — Windows reconnaît Luacord comme app séparée de Discord
...
app.once("ready", () => {
    try {
        ...
        //     Seuls les modules vraiment inutiles pour Luacord sont bloqués.
        ...
        app.once("browser-window-created", () => {
            try {
                ...
                ses.webRequest.onBeforeRequest(
                    ...
                    (details, callback) => {
                        ...
                        if (isBlocked) {
                            ...
                            console.log("[Luacord] Module bloqué (inutile pour Luacord):", url.split("/").slice(-2).join("/"));
                ...
                console.log("[Luacord] Filtre modules 403 activé ✓");
            } catch (e) {
                console.warn("[Luacord] Impossible d'activer le filtre modules:", e.message);
            }
        });
    } catch (e) {
        console.warn("[Luacord] FIX modules 403 failed:", e.message);
    }
});
...
try {
    const lsPath = path.join(luacordData, "Local Storage", "leveldb");
    if (fs.existsSync(lsPath)) {
        ...
        if (corrupted) {
            console.warn("[Luacord] LevelDB localStorage corrompu détecté — réparation...");
            try { fs.rmSync(lsPath, { recursive: true, force: true }); } catch { }
            console.warn("[Luacord] LevelDB supprimé — les données localStorage seront récréées");
        }
    }
} catch (e) { console.warn("[Luacord] LevelDB check failed:", e.message); }

// Modules bundlés dans luacord-dist/modules/
...
try {
    ...
    if (entries.length > 0) {
        ...
        console.log("[Luacord] Modules natifs Discord détectés:", discordNativeModulesPath);
    }
} catch (e) {
    console.warn("[Luacord] Impossible de détecter les modules natifs Discord:", e.message);
}
...
// Priorité aux modules bundlés (portables, dans luacord-dist/modules/)
...
if (discordNativeModulesPath) {
    ...
    } catch (e) { console.warn("[Luacord] Erreur lors du scan des modules natifs:", e.message); }
}
try {
...
console.log("[Luacord] mainAppDirname:", global.mainAppDirname);
...
try {
    ...
    if (fs.existsSync(nativeModulesDir) && !buildInfo.localModulesRoot) {
        ...
        console.log("[Luacord] build_info.json patché → localModulesRoot:", nativeModulesDir);
    }
} catch (e) {
    console.warn("[Luacord] Impossible de patcher build_info.json:", e.message);
}
