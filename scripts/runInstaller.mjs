/*
 * Luacord — Installer via EquilotlCli
 * Télécharge EquilotlCli.exe depuis les releases Equicord et le lance
 * avec les variables d'environnement pointant vers les fichiers Luacord.
 *
 * L'exe affiche une interface graphique permettant de choisir le Discord cible.
 *
 * Usage:
 *   pnpm inject    → installe Luacord dans le Discord choisi
 *   pnpm uninject  → désinstalle Luacord du Discord choisi
...
// EquilotlCli est l'installeur graphique d'Equicord — on le réutilise pour Luacord
...
async function ensureBinary() {
    ...
    if (existsSync(outputFile)) {
        console.log("[Luacord] Installeur déjà présent, utilisation locale.");
        return outputFile;
    }

    console.log("[Luacord] Téléchargement de l'installeur (" + filename + ")...");

    const res = await fetch(BASE_URL + filename, {
        headers: {
            "User-Agent": "Luacord (https://github.com/luacordoff/luacord)"
        }
    });
    ...
    if (process.platform === "darwin") {
        console.log("[Luacord] Sauvegarde du zip...");
        const zip = new Uint8Array(await res.arrayBuffer());
        writeFileSync(downloadName, zip);

        console.log("[Luacord] Extraction du bundle...");
        execSync(`ditto -x -k '${downloadName}' '${FILE_DIR}'`);

        console.log("[Luacord] Suppression de la quarantaine macOS...");
        const logAndRun = cmd => {
    ...
    console.log("[Luacord] Installeur téléchargé avec succès !");
    return outputFile;
}
...
function checkBuild() {
    ...
    if (!existsSync(patcherPath)) {
        console.error("\x1b[31m[Luacord] dist/desktop/patcher.js introuvable !\x1b[0m");
...
function cleanOldLuacord() {
    console.log("[Luacord] Recherche et nettoyage automatique des anciennes installations...");
    ...
    for (const resourcesDir of candidates) {
        ...
        try {
            ...
            // 1. Supprimer le dossier app/ s'il a été créé par l'ancien Luacord
            if (existsSync(appDirPath)) {
                ...
                if (shouldDelete) {
                    console.log(`[Luacord] Suppression de l'ancien dossier app/ dans : ${resourcesDir}`);
            ...
            if (existsSync(backupPath)) {
                ...
                if (isAsarDir) {
                    console.log(`[Luacord] Suppression du dossier app.asar temporaire dans : ${resourcesDir}`);
                    rmSync(appAsarPath, { recursive: true, force: true });
                }

                if (!existsSync(appAsarPath) || isAsarDir) {
                    console.log(`[Luacord] Restauration _app.asar -> app.asar dans : ${resourcesDir}`);
                    ...
                    console.log(`[Luacord] Nettoyage du backup _app.asar obsolète dans : ${resourcesDir}`);
                    rmSync(backupPath, { force: true });
                    cleanedAny = true;
                }
            }
        } catch (e) {
            console.error(`[Luacord] Erreur lors du nettoyage de ${resourcesDir} :`, e.message);
        }
    }

    if (cleanedAny) {
        console.log("[Luacord] Nettoyage des anciennes installations terminé avec succès !");
    } else {
        console.log("[Luacord] Aucune ancienne installation à nettoyer.");
    }
}

// ── Main ─────────────────────────────────────────────────────────────────────
// On nettoie d'abord les anciennes traces de Luacord pour éviter tout conflit ou blocage
cleanOldLuacord();
...
if (isInstall) {
    console.log("[Luacord] Nettoyage des installations précédentes (Vencord/Equicord/Luacord)...");
    try {
        ...
        console.log("[Luacord] Nettoyage terminé.");
    } catch {
        console.log("[Luacord] Aucun mod précédent à nettoyer ou échec du nettoyage.");
    }
}

console.log("[Luacord] Lancement de l'injection...");
...
try {
    execFileSync(installerBin, args, {
        ...
        env: {
            ...
            EQUICORD_DIRECTORY: join(BASE_DIR, "dist", "desktop"),
            EQUICORD_DEV_INSTALL: "1",
            LUACORD_DIRECTORY: join(BASE_DIR, "dist", "desktop")
        }
    });
} catch {
    console.error("[Luacord] Erreur lors de l'injection.");
    process.exit(1);
}
