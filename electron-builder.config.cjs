...
// ─── Configuration de Build Luacord ─────────────────────────────────────────

function killLuacord() {
    const releaseDir = join(__dirname, "release", "luacord-dist");
...
function buildLuacordFromDiscord(discordApp) {
    ...
    console.log("[luacord] Préparation de _app.asar...");
...
app.setPath("userData", path.join(app.getPath("appData"), "Luacord"));
...
`);
    ...
    writeFileSync(join(outApp, "package.json"), JSON.stringify({ name: "luacord", main: "index.js", version: "1.18.0" }, null, 2));
...
`);
    ...
    const luacordPreload = join(__dirname, "luacord-preload.js");
    if (existsSync(luacordPreload)) {
        cpSync(luacordPreload, join(outDist, "preload.js"));
    }
    ...
    const iconSrc = join(__dirname, "luacord.ico");
    if (existsSync(iconSrc)) {
        ...
        try {
            ...
            if (existsSync(rcedit)) {
                execSync(`"${rcedit}" "${discordExe}" --set-icon "${iconSrc}" --set-version-string "ProductName" "Luacord" --set-version-string "FileDescription" "Luacord"`, { stdio: "ignore" });
            }
        } catch (e) { }
    }

    console.log(`[luacord] Build terminé -> ${outDir}`);
}
...
buildLuacordFromDiscord(discord);

module.exports = {
    appId: "com.luacord.app",
    productName: "Luacord",
    copyright: "Copyright 2026 Luacord",
