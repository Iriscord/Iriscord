# ==============================================================================
#  Luacord — Installeur utilisateur (PowerShell autonome)
#  
#  Ce script fait TOUT automatiquement :
#  1. Télécharge EquilotlCli.exe (outil d'injection graphique)
#  2. Télécharge les fichiers Luacord compilés depuis GitHub
#  3. Lance l'interface graphique pour choisir votre Discord cible
#  4. Injecte Luacord dans Discord
#
...
function Write-Banner {
    ...
    Write-Host "  ║          LUACORD  INSTALLER            ║" -ForegroundColor Cyan
...
if (Test-Path $EquilotlExe) {
    ...
    try {
        $head = Invoke-WebRequest -Uri $EquilotlUrl -Method Head -UseBasicParsing `
            -Headers @{ "User-Agent" = "Luacord-Installer/2.0" }
...
# ── [2/3] Télécharger les fichiers Luacord ──────────────────────────────────
Write-Step 2 3 "Téléchargement des fichiers Luacord depuis GitHub..."

try {
    $apiUrl   = "https://api.github.com/repos/$LuacordRepo/releases/latest"
    $release  = Invoke-RestMethod -Uri $apiUrl -UseBasicParsing `
        -Headers @{ "User-Agent" = "Luacord-Installer/2.0"; "Accept" = "application/vnd.github.v3+json" }

    $version  = $release.tag_name
    $distAsset = $release.assets | Where-Object { $_.name -eq "luacord-dist.zip" } | Select-Object -First 1

    if (-not $distAsset) {
        Write-Fail "Fichier 'luacord-dist.zip' introuvable dans la release $version.`n           Contactez le support Luacord."
    }

    Write-Host "          Version : $version" -ForegroundColor DarkGray
    Write-Host "          Téléchargement en cours..." -ForegroundColor DarkGray

    $zipPath = Join-Path $InstallDir "luacord-dist.zip"
    ...
    Write-OK "Luacord $version prêt à être injecté !"
} catch {
    Write-Fail "Échec du téléchargement Luacord.`n           Détail : $_"
}
...
Write-Host "          │  Sélectionnez le Discord où injecter Luacord. │" -ForegroundColor DarkCyan
Write-Host "          └─────────────────────────────────────────────────┘" -ForegroundColor DarkCyan
Write-Host ""
...
Write-Host "  ║  Luacord installé avec succès !                    ║" -ForegroundColor Green
Write-Host "  ║                                                      ║" -ForegroundColor Green
Write-Host "  ║  → Redémarrez Discord pour appliquer Luacord.      ║" -ForegroundColor Green
Write-Host "  ╚══════════════════════════════════════════════════════╝" -ForegroundColor Green
Write-Host ""
Write-Host "  Pour désinstaller : exécutez luacord-uninstall.bat" -ForegroundColor DarkGray
Write-Host ""
