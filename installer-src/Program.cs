...
namespace LuacordInstaller
{
    ...
    public class LauncherForm : Form
    {
        ...
        private LuacordBackend _backend;

        public LauncherForm()
        {
            this.Text = "Luacord Installer";
            ...
            var iconStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("LuacordInstaller.icon.ico");
        ...
        {
            var userDataFolder = Path.Combine(Path.GetTempPath(), "LuacordInstaller_WebView2");
            ...
            )
            {
                ...
                MessageBox.Show(
                    "Microsoft Edge WebView2 Runtime is required to run the Luacord Installer but was not found on your system.\n\n" +
                    "Please download and install it from:\nhttps://aka.ms/webview2\n\n" +
                    "After installing, restart the Luacord Installer.",
            ...
            _backend = new LuacordBackend(this, _webView);
            ...
            using (var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("LuacordInstaller.index.html"))
            using (var reader = new StreamReader(stream))
            {
                ...
                try {
                    using (var iconStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("LuacordInstaller.icon.ico"))
                    {
    ...
    public class LuacordBackend
    {
        ...
        const string GITHUB_REPO = "luacordoff/luacord";
        const string DIST_ZIP = "luacord-dist.zip";

        public LuacordBackend(LauncherForm form, WebView2 webView)
        {
            _form = form;
            _webView = webView;
            ...
            _distDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Luacord", "dist");
        }
        ...
        public async Task<bool> IsInjected(string path)
        {
            return await Task.Run(() => {
                ...
                return Directory.Exists(appDir) && File.Exists(pkgPath) && File.ReadAllText(pkgPath).Contains("\"luacord\"");
            });
        }

        /// <summary>
        /// Returns true if a third-party mod (Vencord, Equicord, OpenAsar) is detected
        /// but Luacord is NOT yet injected.
        /// </summary>
        public async Task<bool> HasThirdPartyMod(string path)
        {
            return await Task.Run(() => {
                ...
                // Already Luacord → not a third-party mod
                if (content.Contains("\"luacord\"")) return false;
        ...
        private async Task EnsureDistAsync()
        {
            ...
            _http.DefaultRequestHeaders.Add("User-Agent", "Luacord-Installer/2.0");
            ...
            var tmpZip = Path.Combine(Path.GetTempPath(), "luacord-dist.zip");
                ...
                using (var fs = new FileStream(tmpZip, FileMode.Create, FileAccess.Write, FileShare.None, 81920, true))
                {
                    ...
                    while ((read = await contentStream.ReadAsync(buffer, 0, buffer.Length)) > 0)
                    {
                        ...
                        if (percent - lastReportedPercent >= 0.5 || percent >= 100.0)
                        {
                            ...
                            SetProgress(overallPercent, "Downloading Luacord...", readMB, totalMB);
                        }
                    }
                }
            }
        ...
        private void DoInject(string resPath)
        {
            ...
            {
                // Déjà Luacord → on réinjecte proprement sans bail-out partiel
            ...
            SetProgress(92, "Configuring Luacord loader...");
            ...
            // Créer le backup Luacord si app.asar existe et qu'on n'a pas encore de backup
        ...
        private void CleanModulePatches(string resPath)
        {
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine($"[Luacord] CleanModulePatches warning: {ex.Message}");
            }
        }
        ...
        private void DoUninject(string resPath)
        {
            ...
            if (Directory.Exists(appDir))
            {
                ...
                if (File.Exists(pkg) && File.ReadAllText(pkg).Contains("\"luacord\""))
                {
            ...
            // 3. Clean up Luacord-specific assets (folders we added)
        ...
        private void WriteLoader(string appDir)
        {
            ...
            // _distDir = %LOCALAPPDATA%\Luacord\dist\
            ...
            File.WriteAllText(Path.Combine(appDir, "package.json"), "{\"name\":\"luacord\",\"main\":\"index.js\"}");
            File.WriteAllText(Path.Combine(appDir, "index.js"),
                ...
                $"if (!fs.existsSync(patcherPath)) throw new Error('[Luacord] patcher.js not found. Expected at: ' + primary);\n" +
                $"require(patcherPath);\n"
            );
        }
