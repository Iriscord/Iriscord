...
all: $(BUILD_DIR)/luacord $(BUILD_DIR)/updater.exe
...
$(BUILD_DIR)/luacord_rc.o: luacord.rc assets/luacord.ico
	mkdir -p $(BUILD_DIR)
	windres luacord.rc -o $@

$(BUILD_DIR)/%.o: %.c
	mkdir -p $(dir $@)
	$(CC) -c -o $@ $(FLAGS) $< -Wall -Wno-unused-function -Wno-misleading-indentation $(OPTS)

$(BUILD_DIR)/luacord: $(OBJS) $(BUILD_DIR)/resources.o $(RC_OBJ) $(BUILD_DIR)/themes/ $(BUILD_DIR)/sounds/
...
uninstall:
	rm -f $(PREFIX)/share/applications/luacord.desktop
	rm -f $(PREFIX)/share/pixmaps/luacord.png
	rm -f $(PREFIX)/share/pixmaps/luacord.svg
	rm -f $(PREFIX)/bin/luacord

install: uninstall
	cp luacord.desktop $(PREFIX)/share/applications/luacord.desktop
	cp assets/icon.svg $(PREFIX)/share/pixmaps/luacord.svg
	cp $(BUILD_DIR)/luacord $(PREFIX)/bin/luacord

run: all
	cp assets/luacord.gschema.xml $(BUILD_DIR)/assets/luacord.gschema.xml
