...
import { BackupAndRestoreTab, CloudTab, IriscordTab, PatchHelperTab, PluginsTab, ThemesTab, UpdaterTab } from "@components/settings/tabs";
...
const settings = definePluginSettings({
    settingsLocation: {
        ...
        description: "Where to put the Iriscord settings section",
        options: [
    ...
    includeIriscordInfoWhenCopying: {
        type: OptionType.BOOLEAN,
        description: "Also copy Iriscord info (Iriscord, Electron, Chromium) when clicking the version info in the bottom left area of the Settings page",
        default: true
    }
});
...
export default definePlugin({
    ...
    get electronVersion() {
        return IriscordNative.native.getVersions().electron || window.legcord?.electron || null;
    },

    get chromiumVersion() {
        try {
            return IriscordNative.native.getVersions().chrome
    ...
    getInfoRows() {
        ...
        const rows = [`Iriscord ${gitHash}${additionalInfo}`];
    ...
    getInfoString() {
        if (!settings.store.includeIriscordInfoWhenCopying) return "";
