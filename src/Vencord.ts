...
import "~plugins";
console.log("%c[Luacord]", "color: #5865f2; font-weight: bold;", "Injection successful! Starting services...");
...
function showGreenUpdateBanner() {
    if (document.getElementById("luacord-core-updater-root")) return;

    const banner = document.createElement("div");
    banner.id = "luacord-core-updater-root";
    ...
    titleSpan.textContent = "🔔 Luacord Update Available!";
    ...
    async function doInstall() {
        ...
        try {
            ...
            setStatus("❌ Installation error. Check your connection. Update will be applied on next Discord close.");
            installing = false;
    ...
    closeBtn.title = "Ignore (update will be installed on Discord close)";
    closeBtn.addEventListener("click", () => {
        if (installing) return; // don't close if installation in progress
        if (countdownTimer) { clearInterval(countdownTimer); countdownTimer = null; }
        banner.remove();
...
function initTrayIpc() {
    ...
    VencordNative.tray.onCheckUpdates(async () => {
        try {
            ...
            if (isOutdated) {
                showNotice("A Luacord update is available!", "View Update", () => openSettingsTabModal(UpdaterTab!));
            } else {
    ...
    VencordNative.tray.onRepair(async () => {
        try {
            ...
            UpdateLogger.error("Failed to repair Luacord", err);
        }
    });
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn(
                ...
                "You are seeing this warning because this is a Development build of Luacord.",
