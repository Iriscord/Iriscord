...
import { platformName } from "@luacordplugins/equicordHelper/utils";
...
export default definePlugin({
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        // Groupe 2 : Message "Luacord developers"
        const luacordGroupIds = [
        ...
        const isLuacordGroup = luacordGroupIds.includes(userId);

        if (!isOriginalDev && !isLuacordGroup) return null;
        if (RelationshipStore.isFriend(userId) || isAnyPluginDev(UserStore.getCurrentUser()?.id)) return null;

        const developerText = isOriginalDev ? "Equicord & Vencord plugin developers" : "Luacord developers";
