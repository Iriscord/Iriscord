...
import * as ghostNative from "../luacordplugins/ghostClient/native";
(async () => {
    try {
        await (ghostNative as any).init(null);
    } catch (e) {
        console.warn("[Luacord] Ghost-server pre-start failed:", e);
    }
})();
...
ipcMain.handle(IpcEvents.WORLD_BOMB_TYPE, async (event, text: string, delay: number = 50) => {
    ...
    const tempDir = mkdtempSync(join(tmpdir(), "luacord-wb-"));
...
function runPowershellScript(psScript: string): Promise<void> {
    ...
    const { join } = require("path");
    const { tmpdir } = require("os");
    const tempDir = mkdtempSync(join(tmpdir(), "luacord-ps-"));
...
ipcMain.handle(IpcEvents.WORLD_BOMB_SEQUENCE, async (
    ...
    const tempDir = mkdtempSync(join(tmpdir(), "luacord-wbs-"));
...
`;
    ...
    const tempDir = mkdtempSync(join(tmpdir(), "luacord-kb-"));
...
const OFFICIAL_UPDATE_URL = "https://github.com/luacordoff/luacord/releases/latest/download/Luacord-Installer.exe";

ipcMain.handle(IpcEvents.LUACORD_DOWNLOAD_AND_RUN, async (_, url: string) => {
    ...
    const tmpPath = path.join(os.tmpdir(), "LuacordUpdate-Setup.exe");
    ...
    const { response } = await dialog.showMessageBox({
        ...
        defaultId: 0,
        title: "Mise à jour Luacord",
        message: "Une mise à jour de Luacord est disponible.",
...
ipcMain.handle(IpcEvents.INSTALL_VB_CABLE, async () => {
    ...
    const tmpDir = path.join(os.tmpdir(), "Luacord-VBCable");
    ...
    try {
        ...
        console.error("[Luacord] VBCable install failed:", err);
