...
const RENDERER_CSS_PATH = join(__dirname, IS_VESKTOP ? "iriscordDesktopRenderer.css" : "renderer.css");
...
ipcMain.handle(IpcEvents.OPEN_PLUGINS_FOLDER, () => shell.openPath(USER_PLUGINS_DIR));

ipcMain.handle(IpcEvents.SAVE_USER_PLUGIN, async (_, pluginName: string, content: string) => {
    const safe = String(pluginName).replace(/[^a-zA-Z0-9_-]/g, "");
    if (!safe) throw new Error("Invalid plugin name");

    const dir = join(USER_PLUGINS_DIR, safe);
    await mkdir(dir, { recursive: true });
    await writeFile(join(dir, "index.tsx"), content, "utf-8");
    return safe;
});
...
ipcMain.handle(IpcEvents.OPEN_MONACO_EDITOR, async () => {
    const title = "Iriscord QuickCSS Editor";
    ...
    const win = new BrowserWindow({
        ...
        webPreferences: {
            preload: join(__dirname, IS_DISCORD_DESKTOP ? "preload.js" : "iriscordDesktopPreload.js"),
