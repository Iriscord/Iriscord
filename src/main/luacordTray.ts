/*
 * Luacord — System Tray (injection mode)
 * Creates a Luacord system tray icon when injected into Discord Desktop.
...
/**
 * Initializes the Luacord system tray icon.
 ...
 */
export function initLuacordTray(injectorPath: string) {
    ...
    app.whenReady().then(() => {
        try {
            ...
            const trayMenu = Menu.buildFromTemplate([
                ...
                { type: "separator" },
                {
                    label: "Uninstall Luacord",
                    click() {
                        const uninstallerCandidates = [
                            join(process.env.LOCALAPPDATA || "", "Programs", "luacord", "Uninstall Luacord.exe"),
                            join(process.env.LOCALAPPDATA || "", "Programs", "Luacord", "Uninstall Luacord.exe"),
                            join(process.env.PROGRAMFILES || "", "Luacord", "Uninstall Luacord.exe"),
                        ];
            ...
            console.log("[Luacord] Tray created ✓", iconPath ?? "(icon from exe)");
        } catch (e) {
            console.error("[Luacord] Failed to create tray:", e);
        }
    });
}
