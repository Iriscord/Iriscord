...
try { _stealthActive = localStorage.getItem("Luacord_stealthMode") === "1"; } catch { }
...
function persistStealth(v: boolean) {
    try { v ? localStorage.setItem("Luacord_stealthMode", "1") : localStorage.removeItem("Luacord_stealthMode"); } catch { }
}

// Éléments NON-React uniquement (titlebar, LuacordAI nav)
...
export function syncStealthBodyClass() {
    try { if (_stealthActive) document.body?.classList.add("luacord-stealth"); else document.body?.classList.remove("luacord-stealth"); } catch { }
...
if (_stealthActive) {
    ...
    try { document.body?.classList.add("luacord-stealth"); } catch { }
}
...
try {
    ...
    window.addEventListener("luacord-stealth-change", () => {
...
export function _notifyStealthChange() {
    ...
    window.dispatchEvent(new Event("luacord-stealth-change"));
}
