...
handleSync(IpcEvents.GET_GIT_HASH, () => Luacord_GIT_HASH);
...
// Handler pour VencordNative.luacord.relaunch() — utilisé par les boutons Restart dans les settings plugins
...
handleSync(IpcEvents.IS_USING_CUSTOM_VENCORD_DIR, () => !!State.store.LuacordDir);
handle(IpcEvents.SHOW_CUSTOM_VENCORD_DIR, async () => {
    const { LuacordDir } = State.store;
    if (!LuacordDir) return;

    const stats = await stat(LuacordDir);
    if (!stats.isDirectory()) return;

    shell.openPath(LuacordDir);
});
