...
export const ExcludedReasons: Record<"web" | "discordDesktop" | "vesktop" | "equibop" | "desktop" | "dev", string> = {
    ...
    dev: "Developer version of Luacord"
};
...
export default function PluginSettings({ premiumOnly = false }: PluginSettingsProps) {
    ...
    const [searchValue, setSearchValue] = useState({ value: "", status: SearchStatus.LUACORD });
    ...
    React.useEffect(() => {
        ...
        window.addEventListener("luacord-tutorial-detected", handler);
        return () => window.removeEventListener("luacord-tutorial-detected", handler);
    }, []);
    ...
    const pluginFilter = useCallback((plugin: typeof Plugins[keyof typeof Plugins], newPluginsSet: Set<string> | null) => {
        ...
        switch (status) {
            ...
            case SearchStatus.ENABLED:
                if (!enabled) return false;
                break;
            case SearchStatus.LUACORD:
                if (!PluginMeta[plugin.name].folderName.startsWith("src/luacordplugins/")) return false;
                break;
            case SearchStatus.OTHERS:
                if (PluginMeta[plugin.name].folderName.startsWith("src/luacordplugins/") || PluginMeta[plugin.name].folderName.startsWith("src/plugins/_")) return false;
    ...
    const { luacordPlugins, othersPlugins, requiredPlugins } = useMemo(() => {
        const luacordPlugins = [] as JSX.Element[];
        ...
        for (const p of sortedPlugins) {
            ...
            if (isRequired) {
                const tooltipText = p.required || !depMap[p.name]
                    ? "This plugin is required for Luacord to function."
                ...
                const isLuacord = folderName.startsWith("src/luacordplugins/");
                const card = (
                    <PluginCard
                        onRestartNeeded={handleRestartNeeded}
                        disabled={false}
                ...
                if (isLuacord) {
                    luacordPlugins.push(card);
                } else {
                    othersPlugins.push(card);
                }
            }
        }
        return { luacordPlugins, othersPlugins, requiredPlugins };
    ...
    const allPlugins = [...luacordPlugins, ...othersPlugins];
    ...
    const luacordVisible = luacordPlugins.slice(0, Math.min(visibleCount, luacordPlugins.length));
    const othersVisible = othersPlugins.slice(0, Math.max(0, visibleCount - luacordPlugins.length));
        ...
        <SettingsTab>
                    ...
                    <ErrorBoundary noop>
                        <Select
                            options={[
                                ...
                                { label: "Show Luacord Plugins", value: SearchStatus.LUACORD },
            ...
            {premiumOnly ? (
                <>
                    ...
                    {luacordPlugins.length || othersPlugins.length
                        ? (
                            <div className={cl("grid")}>
                                {[...luacordVisible, ...othersVisible].length
                                    ? [...luacordVisible, ...othersVisible]
                ...
                <>
                    {luacordPlugins.length > 0 && (
                        <>
                            <HeadingTertiary className={Margins.top20}>Luacord Plugins</HeadingTertiary>
                            <div className={cl("grid")}>
                                {luacordVisible}
                            </div>
                        </>
                    ...
                    {luacordPlugins.length === 0 && othersPlugins.length === 0 && (
                        <ExcludedPluginsList search={search} />
                    )}
