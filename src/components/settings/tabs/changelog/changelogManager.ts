...
const LUACORD_RELEASES_REPO = "luacordoff/luacord";
const LUACORD_REPO_URL = `https://github.com/${LUACORD_RELEASES_REPO}`;
...
export { LUACORD_REPO_URL };

export async function getCommitsSinceLastSeen(
    repoUrl: string,
): Promise<ChangelogEntry[]> {
    // Toujours utiliser le repo Luacord, ignorer le repoUrl d'Equicord
    return fetchCommitsBetween(LUACORD_RELEASES_REPO, "HEAD~10", "HEAD").catch(() => []);
}
