...
import { ContributeModal } from "../../../../luacord/renderer/components/ContributeModal";
...
const DEV_TEAM = [
    {
        ...
        pfp: "https://i.pinimg.com/736x/64/56/06/64560604857f7db3e184009fdb02ca44.jpg",
        description: "Founder and owner of Luacord."
    }
];
...
function DevTeamSection() {
    ...
    return (
        <>
            <div className="vc-luacord-quick-actions">
                <QuickAction
                    Icon={LogIcon}
                ...
                <QuickAction
                    Icon={PlanetIcon}
                    text="Luacord Server"
                    action={() => window.open("https://discord.gg/pE3xn59aZC", "_blank")}
                />
            </div>
            ...
            {showDevs && (
                <div style={{ marginTop: "20px", animation: "slideIn 0.3s ease-out" }}>
                    ...
                    {DEV_TEAM.map(dev => {
                        return (
                            <Card key={dev.id} variant="primary" outline style={{ padding: "16px", marginBottom: "12px" }}>
                                <Flex align={Flex.Align.CENTER} gap="16px">
                                    <Avatar
                                        src={dev.pfp}
                                        size="SIZE_56"
                                    />
                                    <Flex direction={Flex.Direction.VERTICAL} style={{ flex: 1, gap: "4px" }}>
                                        <Heading tag="h3" style={{ marginBottom: "0px", fontSize: "18px" }}>{dev.name}</Heading>
                                        <Heading tag="h4" style={{ opacity: 0.7, marginBottom: "4px", fontSize: "14px", fontWeight: "500" }}>{dev.role}</Heading>
                                        <Paragraph size="sm" style={{ lineHeight: "1.4", opacity: 0.8 }}>{dev.description}</Paragraph>
                                    </Flex>
                                </Flex>
                            </Card>
                        );
...
function useStealthActive() {
    ...
    React.useEffect(() => {
        ...
        window.addEventListener("luacord-stealth-change", handler);
        return () => window.removeEventListener("luacord-stealth-change", handler);
    }, []);
    return active;
}
    ...
    return (
        <>
            ...
            <Paragraph className={Margins.bottom16}>
                {enabled
                    ? "Stealth mode is enabled — all Luacord visual elements are hidden. Shortcut: Ctrl+Shift+H"
                    : "Hides all Luacord visual elements (icons, buttons, context menus) without disabling plugins. Shortcut: Ctrl+Shift+H"}
            </Paragraph>
            <Button
    ...
    return (
        <SettingsTab>
            ...
            {!stealthActive && (<>
                ...
                <Heading className={Margins.top16}>Welcome to Luacord! 🌙</Heading>
                <Paragraph className={Margins.bottom16}>
                    Thank you for using Luacord v1.0.0! This is the first official release of Luacord, bringing you a beautiful and feature-rich Discord experience. Explore the settings below to customize your Discord to your liking.
                </Paragraph>
                ...
                <Paragraph className={Margins.bottom16}>
                    Configure how Luacord behaves and integrates with Discord. These settings affect the Discord client's appearance and behavior.
                </Paragraph>
                ...
                <Paragraph className={Margins.bottom16}>
                    Configure how Luacord handles notifications. You can customize when and how you receive alerts, or view a history of past notifications.
                </Paragraph>
            ...
            <Heading className={Margins.top20}>Stealth Mode</Heading>
            <Paragraph className={Margins.bottom16}>
                Hides all Luacord visual elements without disabling plugins. Shortcut: Ctrl+Shift+H
            </Paragraph>
            <StealthModeButton />
...
export default wrapTab(EquicordSettings, "Luacord Settings");
